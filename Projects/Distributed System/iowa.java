...
public class iowa implements Runnable {
	static ServerSocket s[]=new ServerSocket[3];
	static String state = "RELEASED";
	static String[] array={"kansas.cs.rit.edu","glados.cs.rit.edu","newyork.cs.rit.edu"};
	int port;
	int tid, id;
	...
	public iowa(int num, int id, int p) {
		this.id = num;
		this.tid = id;
		this.port = p;
	}
...
//		bw = new BufferedWriter(fw);
		setFlags();s[0] = new ServerSocket(37000);
		s[1] = new ServerSocket(38000);
		s[2] = new ServerSocket(39000);
		new Thread(new iowa(0,1, 32000)).start();
		new Thread(new iowa(1,1, 35000)).start();
		new Thread(new iowa(2,1, 42000)).start();
		new Thread(new iowa(0,2, 32000)).start();
		new Thread(new iowa(1,2, 35000)).start();
		new Thread(new iowa(2,2, 42000)).start();
		new Thread(new iowa(0,3, 32000)).start();
		new Thread(new iowa(1,4, 35000)).start();
	}
	/*
	 * run()
	 */
