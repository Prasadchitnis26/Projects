...
public class kansas implements Runnable {
	static ServerSocket s[]=new ServerSocket[3];
	static String state = "RELEASED";
	static String[] array={"glados.cs.rit.edu","iowa.cs.rit.edu","newyork.cs.rit.edu"};
	int port;
	int tid, id;
	...
	public kansas(int num, int id, int p) {
		this.id = num;
		this.tid = id;
		this.port = p;
	}
...
//		bw.write("number1 is the 1st written line");
//
//		bw.flush();
		setFlags();s[0] = new ServerSocket(31000);
		s[1] = new ServerSocket(32000);
		s[2] = new ServerSocket(33000);
		new Thread(new kansas(0,1, 34000)).start();
		new Thread(new kansas(1,1, 37000)).start();
		new Thread(new kansas(2,1, 40000)).start();
		new Thread(new kansas(0,2, 34000)).start();
		new Thread(new kansas(1,2, 37000)).start();
		new Thread(new kansas(2,2, 40000)).start();
		new Thread(new kansas(0,3, 34000)).start();
		new Thread(new kansas(1,4, 37000)).start();
	}
