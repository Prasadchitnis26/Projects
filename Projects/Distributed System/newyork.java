...
public class newyork implements Runnable {
	...
	static String[] array={"kansas.cs.rit.edu","glados.cs.rit.edu","iowa.cs.rit.edu"};
	int port;
	int tid, id;
	...
	public newyork(int num, int id, int p) {
		this.id = num;
		this.tid = id;
		this.port = p;
	}
...
//		bw = new BufferedWriter(fw);
		setFlags();s[0] = new ServerSocket(40000);
		s[1] = new ServerSocket(41000);
		s[2] = new ServerSocket(42000);
		new Thread(new newyork(0,1, 33000)).start();
		new Thread(new newyork(1,1, 36000)).start();
		new Thread(new newyork(2,1, 39000)).start();
		new Thread(new newyork(0,2, 33000)).start();
		new Thread(new newyork(1,2, 36000)).start();
		new Thread(new newyork(2,2, 39000)).start();
		new Thread(new newyork(0,3, 33000)).start();
		new Thread(new newyork(1,4, 36000)).start();
	}
	/*
	 * run()
	 */
