 ...
 **/
public class nebraska2 implements Runnable {
	int id;
	static String state = "RELEASED";
	static LinkedList<String> list = new LinkedList<String>();
	static int forward = 0;
	static String myAddr = "nebraska.cs.rit.edu", holder = "iowa.cs.rit.edu";
	static boolean transmitted = false, hasToken = false, addedToList = false, reqSent = false;
	static ServerSocket s;

	public nebraska2(int i) {
		this.id = i;
	}

	public static void main(String args[]) throws IOException {
		s = new ServerSocket(35500);
		new Thread(new nebraska2(0)).start();
		new Thread(new nebraska2(1)).start();
		new Thread(new nebraska2(2)).start();
		new Thread(new nebraska2(3)).start();
	}
