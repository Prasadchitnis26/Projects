 ...
 **/
public class vermont2 implements Runnable {
	int id;
	static String state = "RELEASED";
	static LinkedList<String> list = new LinkedList<String>();
	static int forward = 0;
	static String myAddr = "vermont.cs.rit.edu", holder = "nebraska.cs.rit.edu";
	static boolean transmitted = false, hasToken = false, addedToList = false, reqSent = false;
	static ServerSocket s;

	public vermont2(int i) {
		this.id = i;
	}

	public static void main(String args[]) throws IOException {
		s = new ServerSocket(35500);
		new Thread(new vermont2(0)).start();
		new Thread(new vermont2(1)).start();
		new Thread(new vermont2(2)).start();
		new Thread(new vermont2(3)).start();
	}
