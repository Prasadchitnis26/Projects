 ...
 **/
public class kansas2 implements Runnable {
	int id;
	static String state = "RELEASED";
	static LinkedList<String> list = new LinkedList<String>();
	static int forward = 0;
	static String myAddr = "kansas.cs.rit.edu", holder = "glados.cs.rit.edu";
	static boolean transmitted = false, hasToken = false, addedToList = false, reqSent = false;
	static ServerSocket s;

	public kansas2(int i) {
		this.id = i;
	}

	public static void main(String args[]) throws IOException {
		s = new ServerSocket(35500);
		new Thread(new kansas2(0)).start();
		new Thread(new kansas2(1)).start();
		new Thread(new kansas2(2)).start();
		new Thread(new kansas2(3)).start();
	}
